...
def check_amp(model):
    ...
    def amp_allclose(m, im):
        ...
        # m is an object of class YOLO(Model), Model is defined in engine/model.py, YOLO is defined in models/yolo/model.py
        # m() calls the predictor method defined in Model, m.model is DetectionModel(BaseModel) defined in nn/tasks.py
        # the predictor is DetectionPredictor(BasePredictor) defined in models/yolo/detect/predict.py, BasePredictor is defined in engine/predictor.py
        # DetectionPredictor only modifies the postprocess method, the remaining methods and attributes are all inheritted from BasePredictor
        # the predictor calls __call__() -> stream_inference() -> setup_model()
        # now the predictor initializes its self.model as an AutoBackend object, defined in nn/autobackend.py
        # it then calls self.model.warmup() !!! WE MODIFIED THE CHANNELS HERE !!!
        # it then calls preprocess() !!! WE MODIFIED THE CHANNELS HERE !!!
        # it then calls inference(), which calls self.model() (i.e., the AutoBackend object's forward function)
        # so in essence, we need the let the predictor know when to enter AMP checking mode
        # so we first need to define an amp_check attribute for Model
        # then define an attribute for BasePredictor
        # set the attribute for BasePredictor to be True if True in Model when initializing the predictor
        # then in BasePredictor, use condition control-flow to enter AMP checking mode when needed
        #print(f'[debug] model predictor: {m.predictor}, model.model: {m.model}')
        m.amp_check = True
        print('[debug] begin FP32 inference')
        a = m(im, device=device, verbose=False)[0].boxes.data  # FP32 inference
        with torch.cuda.amp.autocast(True):
            print('[debug] begin AMP inference')
